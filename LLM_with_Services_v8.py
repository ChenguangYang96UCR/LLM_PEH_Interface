...
def wild_search_by_keywords(key_word, relation=''):
    ...
    if relation == '':
        ...
        with open(work_path + 'Extracted_Triples.txt', 'a', encoding='utf-8') as file:
...
def getQuestion_answer(Service_list, st):
    ...
    for Service in Service_list:
        triples = wild_search_by_keywords(Service)
...
Output please start with bold service name: {Service}.
input knowledge graph triples: {triples[0]} {triples[1]} {triples[2]}
"""
            ...
            # print('Answer: {0} '.format(response))
            # all_information.append(response)
...
def read_data(df):
    ...
    for index, row in df.iterrows():
        ...
        service_list.append(row['Service_Name'])
                ...
                try:
                    ...
                    if classified_service_type != "Other":
                        ...
                        if not location_info.empty:
                            ...
                            extract_services = []
                            for service in service_list:
                                start_brasket = service.find('(')
                                end_brasket = service.find(')', start_brasket + 1)
                                extract_service = service[start_brasket+1:end_brasket]
                                extract_services.append(extract_service)
                                if len(extract_services) >= 2:
                                    break
