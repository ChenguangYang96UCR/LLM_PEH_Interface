...
def read_data(df):
    ...
    for index, row in df.iterrows():
        ...
        try:
            # Safely parse 'Main_Services' and 'Other_Services' fields
            main_services = ast.literal_eval(row['Main_Services']) if pd.notna(row['Main_Services']) else []
            other_services = ast.literal_eval(row['Other_Services']) if pd.notna(row['Other_Services']) else []
        except ValueError as e:
            print(f"Error parsing services data at row {index}: {e}")
            main_services, other_services = [], []
...
api_key = 'sk-proj-yu46NPL9HS19sGFQApnWT3BlbkFJNDYsqs3gsWPJFpvmGwsZ'  # Replace this with your actual OpenAI API key
